...
void GreeView::render()
{
    ...
    {
        if(acgree->getMode()==kGreeCool){acgree->setMode(kGreeHeat);}
        else{acgree->setMode(kGreeCool);}
    }
    disp_buffer->setTextSize(3);
    disp_buffer->setTextColor(grey, TFT_BLACK);
    if(acgree->getMode()==kGreeHeat){disp_buffer->drawString("HAET", 25, 25, 2);}
    if(acgree->getMode()==kGreeCool){disp_buffer->drawString("COOL", 25, 25, 2);}
    disp_buffer->setTextSize(5);
    disp_buffer->setTextColor(TFT_WHITE, TFT_BLACK);
